...
pub fn read_varint<R: Read>(reader: &mut R) -> Result<u64> {
    let mut buf = [0u8; 1];
    reader.read_exact(&mut buf).map_err(|e| ConsensusError::Serialization(format!("IO error: {}", e).into()))?;
...
pub fn write_varint<W: Write>(writer: &mut W, value: u64) -> Result<usize> {
    match value {
        0..=0xfc => {
            writer.write_all(&[value as u8]).map_err(|e| ConsensusError::Serialization(format!("IO error: {}", e).into()))?;
            Ok(1)
        }
        0xfd..=0xffff => {
            writer.write_all(&[0xfd]).map_err(|e| ConsensusError::Serialization(format!("IO error: {}", e).into()))?;
            writer.write_all(&(value as u16).to_le_bytes()).map_err(|e| ConsensusError::Serialization(format!("IO error: {}", e).into()))?;
            Ok(3)
        }
        0x10000..=0xffff_ffff => {
            writer.write_all(&[0xfe]).map_err(|e| ConsensusError::Serialization(format!("IO error: {}", e).into()))?;
            writer.write_all(&(value as u32).to_le_bytes()).map_err(|e| ConsensusError::Serialization(format!("IO error: {}", e).into()))?;
            Ok(5)
        }
        _ => {
            writer.write_all(&[0xff]).map_err(|e| ConsensusError::Serialization(format!("IO error: {}", e).into()))?;
            writer.write_all(&value.to_le_bytes()).map_err(|e| ConsensusError::Serialization(format!("IO error: {}", e).into()))?;
            Ok(9)
        }
    }
}
...
mod tests {
    ...
    fn test_varint_roundtrip() {
        let test_values = vec![0, 1, 0xfc, 0xfd, 0xffff, 0x10000, 0xffff_ffff, 0x1_0000_0000, u64::MAX];
