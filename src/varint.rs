...
pub fn write_varint<W: Write>(writer: &mut W, value: u64) -> Result<usize> {
    match value {
        0..=0xfc => {
            writer.write_all(&[value as u8]).map_err(|e| ConsensusError::InvalidInput(format!("IO error: {}", e)))?;
            Ok(1)
        }
        0xfd..=0xffff => {
            writer.write_all(&[0xfd]).map_err(|e| ConsensusError::InvalidInput(format!("IO error: {}", e)))?;
            writer.write_all(&(value as u16).to_le_bytes()).map_err(|e| ConsensusError::InvalidInput(format!("IO error: {}", e)))?;
            Ok(3)
        }
        0x10000..=0xffff_ffff => {
            writer.write_all(&[0xfe]).map_err(|e| ConsensusError::InvalidInput(format!("IO error: {}", e)))?;
            writer.write_all(&(value as u32).to_le_bytes()).map_err(|e| ConsensusError::InvalidInput(format!("IO error: {}", e)))?;
            Ok(5)
        }
        _ => {
            writer.write_all(&[0xff]).map_err(|e| ConsensusError::InvalidInput(format!("IO error: {}", e)))?;
            writer.write_all(&value.to_le_bytes()).map_err(|e| ConsensusError::InvalidInput(format!("IO error: {}", e)))?;
            Ok(9)
        }
    }
}
