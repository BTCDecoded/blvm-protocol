...
//! Unit tests for UTXO commitments (moved from blvm-consensus to avoid a dev-dependency cycle).
...
mod tests {
    ...
    fn test_verify_supply_function() {
        ...
        let commitment = UtxoCommitment::new(
            [0; 32], 5000000000, // 50 BTC at genesis
            1, 0, [0; 32],
        );
    ...
    fn test_commitment_verify_count() {
        let commitment = UtxoCommitment::new([0; 32], 1000, 5, 0, [0; 32]);
